      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex md:hidden items-center space-x-2">
            <button
              onClick={toggleTheme}
              className="p-2 rounded-full bg-gray-100 dark:bg-gray-800 text-[#1F2937] dark:text-gray-200 mr-2"
              aria-label="Toggle theme"
            >
              {mounted && theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
            </button>
            ...
            <nav className="flex flex-col space-y-4">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`text-sm font-medium py-2 transition-colors hover:text-[#4CAF50] ${
                    pathname === link.href ? "text-[#4CAF50]" : "text-[#1F2937] dark:text-gray-200"
                  }`}
                >
                  {link.name}
                </Link>
              ))}
              <Button asChild size="sm" className="bg-[#4CAF50] hover:bg-green-700 text-white w-full">
